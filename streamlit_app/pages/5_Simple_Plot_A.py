...
with opt_col:
    ...
    st.subheader("Ausreißer einfügen")
    pct = st.slider("Anteil [%]", 0.0, 20.0, 3.0, 0.5)
    low_val = st.number_input("Niedriger Wert", value=0.0)
    high_val = st.number_input("Hoher Wert", value=700.0)
    inj_seed = st.number_input("Inject Seed", value=1, step=1)
